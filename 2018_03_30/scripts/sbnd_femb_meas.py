...
#        self.femb_config.configAdcAsic()
        ...
        print ("Saving baseline images...")
        
        for i in range(settings.chip_num):
            data = self.femb_config.femb.get_data_packets(ip = settings.CHIP_IP[i], 
                                                               data_type = "int", num = 25, header = False)
#            print ("Chip {}".format(i))
            chip_data[i] = self.analyze.UnpackData(path = "data", data = data, return_data = True)
            
            
        scripts.raw_process2.print_pulses(self.path, chip_data, "baseline")
...
#        self.femb_config.femb.write_reg(16, 0x002, board = "femb")
#        
#        time.sleep(0.1)
#        chip_data = [[],[],[],[]]
#        
...
#            
#            
#        scripts.raw_process2.print_pulses(self.path, chip_data, "pulses")
#            
#        #Bring everything back the way it was            
#        self.femb_config.femb.write_reg(self.femb_config.REG_TEST_PULSE, reg_5_original, "femb")
#        
...
#        self.femb_config.configAdcAsic()
    ...
    def intdac_cali(self):
        ...
        self.femb_config.fe_reg.set_fe_board(sts=1,sg=self.current_sg[0], st=self.current_st[0], snc=1,
                                                      sdacsw1=0, sdacsw2=1, sdac=32)

#        self.femb_config.configAdcAsic()
        self.femb_config.configFeAsic(output = "suppress")
